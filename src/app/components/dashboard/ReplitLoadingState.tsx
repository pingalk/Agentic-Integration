...
export const ReplitLoadingState: React.FC = () => {
  const [currentStep, setCurrentStep] = useState(0);

  useEffect(() => {
    // Progress through steps every 1.75 seconds (3.5s total / 2 steps)
    const interval = setInterval(() => {
      setCurrentStep(prev => {
        if (prev < REPLIT_STEPS.length - 1) {
          return prev + 1;
        }
        return prev;
      });
