...
// --- Test Payment Success Artifact Component ---
const TestPaymentSuccessArtifact = ({ data, onSuggestionClick, isLast, highlightedSuggestionIndex = null }: any) => {
  return (
    <motion.div
      initial={{ opacity: 0, y: 10 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.4, ease: [0.4, 0, 0.2, 1] }}
      className="flex flex-col gap-4 w-full max-w-[531px]"
    >
      {/* Heading */}
      <div className="flex flex-col gap-2">
        <h3 className="text-[18px] font-semibold leading-[24px] tracking-[-0.2px] text-[#020202]">
          {data.headline}
        </h3>
        <p className="text-[16px] leading-[24px] tracking-[-0.2px] text-[#40566d]">
          {data.subtext}
        </p>
      </div>

      {/* Success Card */}
      <div className="relative backdrop-blur-[5.5px] bg-gradient-to-b from-white to-[#f0f0f0] border-[1.5px] border-[rgba(0,0,0,0.1)] rounded-[16px] shadow-[0px_8px_48px_4px_rgba(59,96,181,0.1)] overflow-hidden p-6">
        {/* Inner shadow for depth */}
        <div className="absolute inset-0 pointer-events-none rounded-[inherit] shadow-[inset_0px_-2px_0px_1px_white]" />

        <div className="relative flex flex-col gap-3">
          {/* Status Badge */}
          <div className="inline-flex items-center gap-2 self-start px-3 py-1.5 bg-[#04c982]/10 rounded-full">
            <div className="w-2 h-2 rounded-full bg-[#04c982]" />
            <span className="text-[13px] font-medium text-[#04c982]">{data.status}</span>
          </div>

          {/* Amount */}
          <div className="text-[32px] font-semibold leading-[40px] tracking-[-0.5px] text-[#020202]">
            ₹ {data.amount}
          </div>

          {/* Email */}
          <div className="text-[14px] leading-[20px] text-[#40566d]">
            {data.email}
          </div>
        </div>
      </div>
...
export const RayMessageRenderer = ({ data, onSuggestionClick, onRowClick, isLast = true, highlightedSuggestionIndex = null, onMiniCardClick, onMiniCardAnimationComplete, onStreamComplete, animatingCardId, personaId, kycPanNumber, kycWebsite, kycBusinessName, kycBusinessModel, kycBankAccount, isKYCReviewModalOpen, showFinalVideo, onKYCPanelSettled, onKYCPanelClosed, onOpenReplitInterface }: { data: RayResponseData; onSuggestionClick?: (suggestion: string) => void; onRowClick?: (rowData: any) => void; isLast?: boolean; highlightedSuggestionIndex?: number | null; onMiniCardClick?: (formId: string, sourceRect?: SourceRect) => void; onMiniCardAnimationComplete?: (formId: string) => void; onStreamComplete?: () => void; animatingCardId?: string | null; personaId?: string; kycPanNumber?: string; kycWebsite?: string; kycBusinessName?: string; kycBusinessModel?: string; kycBankAccount?: string; isKYCReviewModalOpen?: boolean; showFinalVideo?: boolean; onKYCPanelSettled?: () => void; onKYCPanelClosed?: () => void; onOpenReplitInterface?: () => void }) => {
  ...
  if (data.artifact?.type === 'test_payment_success') {
    return (
      <div className="w-full animate-fade-in-up">
        <TestPaymentSuccessArtifact
          data={data.artifact.data}
          onSuggestionClick={onSuggestionClick}
          isLast={isLast}
          highlightedSuggestionIndex={highlightedSuggestionIndex}
